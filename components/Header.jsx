                    ...
                    Learn more
								...
								{user ? (
									<li className="flex items-center relative ml-1 mr-1">
										{/** User CTA */}
										<div
											className="text-sm font-medium flex flex-row items-center space-x-4 rounded hover:bg-slate-300/[.2] px-3 py-2 cursor-pointer"
								...
								<li className="ml-3">
									...
									>
										Genesis Chat <PanelRight style={{ marginLeft: 5 }} />
										{/** Show unread indicator if any */}
										{hasUnreadMessages && (
											<div
												className="bg-red-500 h-2.5 w-2.5 rounded-full"
												style={{ marginLeft: 6 }}
											></div>
										)}
									</div>
	...
	return (
		<div className="flex flex-row ml-1" onClick={() => setShowNotifPane(true)}>
			<div className="flex flex-row text-gray-900 text-brand-hover  rounded hover:bg-slate-300/[.2] px-3 py-2 cursor-pointer">
				...
				{countNewNotifs > 0 && (
					<div
						className="bg-red-500 rounded-full text-white font-bold py-0.5 px-1.5 text-xs"
						style={{ marginLeft: 6 }}
					>
						{countNewNotifs}
					</div>
...
const NotificationsPane = ({ setCountNewNotifs, hide }) => {
	...
	return (
		<div className="absolute top-[0px] right-[0px] py-10 z-50 w-[355px]">
			<div
				className="text-sm shadow-md bg-white border border-gray-200 rounded-md flex flex-col w-full divide-y max-h-[600px] overflow-y-scroll"
				ref={wrapperRef}
			>
	...
	return (
		...
		>
			...
			{(notification.family == "reply_to" ||
				...
				<div
					className={`border border-slate-200 p-2 rounded-md shadow-md mt-1.5 ${
	...
	return (
		<>
			<div className="absolute top-[0px] right-[0px] py-10 z-50 w-[165px]">
				...
				>
					<div
						className="text-primary font-medium hover:bg-gray-50 cursor-pointer rounded py-1.5 px-2"
				...
				<div className="relative flex w-full flex-col flex-start">
					<div className="relative flex items-center">
						<input
							id="search"
							type="search"
							className="bg-white placeholder:text-slate-500 text-slate-900 border border-slate-200 py-1 w-full pl-10 rounded-full text-sm"
							placeholder="Search..."
							value={search}
								...
								{loading ? (
									...
									<>
										{posts && posts.length > 0 ? (
												...
												{posts.map((post, key) => {
													if (post.content.master) {
														return (
															<Link
																key={post.stream_id}
																className="text-secondary font-medium hover:bg-gray-50 cursor-pointer rounded py-1.5 px-2"
											...
											<div className="w-full text-tertiary text-sm text-center p-3">
		...
		>
			<div className="fixed inset-0 overflow-hidden">
				...
				<div className="absolute inset-0 overflow-hidden cursor-pointer">
					<div
						className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10 cursor-default"
						ref={wrapperRef}
					>
						<div
...
const BackgroundWrapper = ({ hide, children }) => {
	...
	return (
		...
		>
			<div className="fixed inset-0 overflow-hidden">
				<div
					className="fixed inset-0 bg-gray-600 bg-opacity-75 transition-opacity bg-blur cursor-pointer"
					onClick={() => hide()}
				></div>
				{children}
			</div>
